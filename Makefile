...
.PHONY: help build test crap mutate e2e lint doclint clean
...
test: ## 単体テストを実行し、合計カバレッジが80%未満なら失敗する（cmd/とe2e/は対象外）
	@mkdir -p bin
...
mutate: ## internal/domain 配下にmutation testingを実行し、生存mutantがあれば失敗する
	go run $(GREMLINS) unleash ./internal/domain/... --threshold-efficacy 100 --threshold-mcover 100

e2e: build ## ビルドしたバイナリで主要導線のE2Eテストを実行する
	EGOV_BIN=$(CURDIR)/$(BIN) go test -count=1 ./e2e/...

lint: ## go vet と gofmt の差分確認
	go vet ./...
