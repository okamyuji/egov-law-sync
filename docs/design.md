...
要望の直接の対象である税法は、e-Gov法令APIの対象範囲に含まれます。APIは日本の現行法令全体（憲法、法律、政令、府省令、規則）を対象にしており、所得税法、法人税法、消費税法、相続税法、国税通則法、租税特別措置法などの本則と、その施行令・施行規則を取得できます。この仕組みで税法について得られるものと、得られないものは次のとおりです。
...
ActionsのcronはUTCで書きます。分を正時から7分ずらしているのは、正時はActionsのscheduleの待ち行列が混み、起動が1時間以上遅れることがあるためです（2026-09-12の日次は07:00の予定に対し08:52 JSTに起動しました）。

| ジョブ | JST | cron（UTC） |
|---|---|---|
| daily | 毎日07:07 | `7 22 * * *` |
| weekly | 日曜08:07 | `7 23 * * 6` |
...
- 22時台の更新がその日のv1の一覧に入るのか翌日分に入るのか、また前日分のv1が翌朝07:07 JSTの時点で確定しているかは未確認です。日次はv1を対象範囲の2日前から取り直し、切替と新規の法令は/law_revisionsを引き直すので、どちらでも翌日以降に取り込まれます。
...
`--law-id`は法令IDを厳密に指定します。`--law-title`は現時点の正式名称または略称を完全一致で一件に解決し、保存するときは法令IDを使用します。候補がゼロまたは複数なら終了コード2と候補の一覧を返します。改称後も同じ対象を維持するため、継続実行では`--law-id`を使用します。

全件manifestと対象別manifestを同じディレクトリで混ぜません。最初に対象別の`bootstrap`を実行すると`scope.json`に法令IDを保存します。以後の`bootstrap`、`daily`、`weekly`は同一IDだけを受け付け、異なるIDや指定なしの場合は取得前に終了コード2で拒否します。既存の全件`laws.csv`がある場所では対象別の初期化を拒否します。対象を変える場合は新しい`EGOV_MANIFEST_DIR`を指定します。
